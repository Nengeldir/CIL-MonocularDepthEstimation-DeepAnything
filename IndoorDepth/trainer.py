...
class Trainer:
    def __init__(self, options):
        ...
        datasets_dict = {"nyu": datasets.NYUDataset,
                         "deep_anything": datasets.DeepAnythingDataset}  # Add this line
    ...
    def compute_losses(self, inputs, outputs):
        """Compute the reprojection, smoothness, and scale-invariant RMSE losses for a minibatch."""
        losses = {}
        total_loss = 0
        ...
        for scale in self.opt.scales:
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                outputs[("reprojection_losses_virtual", frame_id, scale)] = self.compute_reprojection_loss(pred, target)
                loss += outputs[("reprojection_losses_virtual", frame_id, scale)].mean()
            ...
            # Scale-invariant RMSE loss
            depth_pred = outputs[("depth", 0, scale)]
            depth_gt = inputs["depth_gt"]

            if depth_gt is not None:
                mask = depth_gt > 0  # Mask invalid depth values
                log_diff = torch.log(depth_pred[mask]) - torch.log(depth_gt[mask])
                si_rmse = torch.sqrt((log_diff ** 2).mean() - (log_diff.mean() ** 2))
                loss += self.opt.si_rmse_weight * si_rmse
                losses["si_rmse_loss/{}".format(scale)] = si_rmse
