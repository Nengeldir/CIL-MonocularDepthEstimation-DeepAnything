...
from src.models import AdvancedUNEt, DepthAnythingSmallPretrained
from src.transforms import target_transform
from src.datasets import DepthDataset
from src.loss import scaleinvariant_RMSE, CombinedLoss
...
MODEL_CLASS_LIST = [
    (DepthAnythingSmallPretrained, scaleinvariant_RMSE(), 0),
    (AdvancedUNEt, CombinedLoss(0.5, 0.25, 0.25), 0),
...
def main():
    ...
    for MODEL_CLASS, LOSS, START_EPOCH in MODEL_CLASS_LIST:
        ...
        # Create train data loader without shuffle, so we can consistently get the same pictures for visualisation each epoch
        ...
        model = train_model(model, train_loader, val_loader, criterion, optimizer, NUM_EPOCHS, DEVICE, results_dir=results_dir, train_pictures_loader=train_pictures_loader, start_epoch=START_EPOCH, val_criterion=scaleinvariant_RMSE())
